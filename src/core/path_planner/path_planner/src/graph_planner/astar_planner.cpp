...
{
  R_INFO << "Start position (world frame): (" << start.x() << ", " << start.y() << ", theta=" << start.theta() << ")";
  R_INFO << "Goal position (world frame): (" << goal.x() << ", " << goal.y() << ", theta=" << goal.theta() << ")";
  
  double m_start_x, m_start_y, m_goal_x, m_goal_y;
  if ((!validityCheck(start.x(), start.y(), m_start_x, m_start_y)) ||
      (!validityCheck(goal.x(), goal.y(), m_goal_x, m_goal_y)))
  {
    R_ERROR << "Start or goal position validation failed! Start map pos: (" << m_start_x << ", " << m_start_y << "), Goal map pos: (" << m_goal_x << ", " << m_goal_y << ")";
    return false;
  }
  R_INFO << "Start position (map frame): (" << m_start_x << ", " << m_start_y << ")";
  R_INFO << "Goal position (map frame): (" << m_goal_x << ", " << m_goal_y << ")";
  ...
  while (!open_list.empty())
  {
    ...
    {
      R_INFO << "Goal found! Expanded nodes: " << expand.size() << ", Closed list size: " << closed_list.size();
      const auto& backtrace = _convertClosedListToPath<Node>(closed_list, start_node, goal_node);
      R_INFO << "Path length: " << backtrace.size() << " nodes";
      ...
      R_INFO << "A* path planning succeeded! Final path has " << path.size() << " waypoints";
      return true;
    }
  ...
  R_ERROR << "A* path planning failed! Open list became empty. Expanded nodes: " << expand.size() << ", Closed list size: " << closed_list.size() << ", Open list size: " << open_list.size();
  return false;
}
